...
exports.getFreelancerProposals = async (req, res) => {
  ...
  try {
    ...
    stats.avg_proposed_budget =
      proposals.length > 0 ? totalBudget / proposals.length : 0;
...
exports.acceptProposal = async (req, res) => {
  ...
  try {
    ...
    if (!proposal) {
      return res
        .status(404)
        .json({ success: false, error: "Propuesta no encontrada" });
    }
    ...
    if (proposal.project.deadline) {
      try {
        ...
        console.log(
          "✅ Evento creado en calendario para el proyecto:",
          proposal.project.title
        );
      } catch (calendarError) {
        console.warn(
          "⚠️ No se pudo crear el evento en calendario:",
    ...
    res
      .status(500)
      .json({ success: false, error: "Error al aceptar propuesta" });
  }
};
...
exports.rejectProposal = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Error al rechazar propuesta" });
  }
};

// Obtener propuestas para proyectos de un Project Manager
exports.getProposalsForProjectManager = async (req, res) => {
  try {
    const { managerId } = req.params;

    console.log(`📋 Obteniendo propuestas para manager ${managerId}`);

    // Obtener propuestas de proyectos que pertenecen a este manager
    const proposals = await prisma.project_proposals.findMany({
      where: {
        project: {
          client_id: parseInt(managerId),
        },
      },
      include: {
        project: {
    ...
    // Obtener detalles de usuario para nombres completos
    const freelancerIds = proposals.map((p) => p.freelancer_id);
    const userDetails = await prisma.user_details.findMany({
      where: {
        user_id: { in: freelancerIds },
      },
    });

    // Crear map de detalles de usuario
    ...
    const formattedProposals = proposals.map((proposal) => ({
      ...
      freelancer_name:
        userDetailsMap[proposal.freelancer_id] ||
        proposal.login_credentials.username,
      cover_letter: proposal.cover_letter,
      proposed_budget: proposal.proposed_budget,
      estimated_time: proposal.delivery_time,
      portfolio_url: proposal.portfolio_links?.[0] || null,
      status: proposal.status,
      created_at: proposal.created_at,
    }));

    console.log(`✅ Enviando ${formattedProposals.length} propuestas`);
...
// Obtener contratos activos de un usuario (propuestas con status 'accepted')
// Obtener contratos activos de un usuario (propuestas con status 'accepted')
exports.getUserContracts = async (req, res) => {
  try {
    ...
    const contracts = await prisma.project_proposals.findMany({
      where: {
        status: "accepted",
        OR: [
          { freelancer_id: parseInt(userId) },
          { project: { client_id: parseInt(userId) } },
        ],
      },
      include: {
        project: true,
    ...
    console.log(`✅ Enviando ${contracts.length} contratos`);
    res.json(contracts);
  } catch (error) {
